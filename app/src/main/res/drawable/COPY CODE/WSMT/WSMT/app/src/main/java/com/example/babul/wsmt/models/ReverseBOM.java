...
public class ReverseBOM implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    public static Creator<ReverseBOM> getCREATOR() {
        return CREATOR;
    }

    private ReverseBOM(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<ReverseBOM> CREATOR = new Creator<ReverseBOM>() {
        @Override
        public ReverseBOM createFromParcel(Parcel in) {
            return new ReverseBOM(in);
        }

        @Override
        public ReverseBOM[] newArray(int size) {
            return new ReverseBOM[size];
        }
    };
